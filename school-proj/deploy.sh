...
# 🚀 اسکریپت دیپلوی کامل پروژه School-Proj
# این اسکریپت همه چیز رو از صفر تا صد مدیریت می‌کنه
# دامنه: sch.ahmadreza-avandi.ir
# پورت‌ها: Next.js:3003, Nest.js:3002, Python:5001, MySQL:3307, Redis:6380
...
if [ -f "nginx-config.conf" ]; then
    ...
    # بکاپ از کانفیگ قبلی اگر وجود داشته باشد
    if [ -f "/etc/nginx/sites-available/school-proj" ]; then
        sudo cp /etc/nginx/sites-available/school-proj /etc/nginx/sites-available/school-proj.backup.$(date +%Y%m%d_%H%M%S)
        print_info "بکاپ از کانفیگ قبلی گرفته شد"
    fi
    ...
    if sudo nginx -t; then
        ...
        print_info "در حال بازگردانی کانفیگ قبلی..."
        if [ -f "/etc/nginx/sites-available/school-proj.backup.$(date +%Y%m%d_%H%M%S)" ]; then
            sudo cp /etc/nginx/sites-available/school-proj.backup.$(date +%Y%m%d_%H%M%S) /etc/nginx/sites-available/school-proj
