...
import { Lead, LeadTemperature, PipelineHistoryEntry } from './sales-pipeline-types';
...
export class LeadTemperatureService {
  ...
  constructor(config: TemperatureConfig = DEFAULT_TEMPERATURE_CONFIG) {
    this.config = config;
  }
   ...
   */
  calculateLeadTemperature(lead: Lead): LeadTemperature {
   ...
   */
  async updateLeadTemperature(lead: Lead): Promise<LeadTemperature> {
   ...
   */
  async batchUpdateTemperatures(leads: Lead[]): Promise<Map<string, LeadTemperature>> {
    const results = new Map<string, LeadTemperature>();
   ...
   */
  async getLeadsNeedingTemperatureUpdate(): Promise<Lead[]> {
   ...
   */
  async scheduleTemperatureUpdates(): Promise<void> {
   ...
   */
  getTemperatureStats(leads: Lead[]): { hot: number; warm: number; cold: number } {
   ...
   */
  getLeadsByTemperature(leads: Lead[], temperature: LeadTemperature): Lead[] {
   ...
   */
  validateLeadForTemperatureCalculation(lead: Lead): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  private getDaysSinceLastInteraction(lead: Lead): number {
  ...
  private getDaysSinceLastFollowup(lead: Lead): number {
  ...
  private async persistTemperatureUpdate(leadId: string, temperature: LeadTemperature): Promise<void> {
  ...
  private async logTemperatureChange(
    leadId: string, 
 ...
 */
export const leadTemperatureService = new LeadTemperatureService();
...
export const TemperatureUtils = {
   ...
   */
  getTemperatureIcon(temperature: LeadTemperature): string {
    switch (temperature) {
      case 'hot':
        return '🔥';
      case 'warm':
   ...
   */
  getTemperatureDisplayName(temperature: LeadTemperature): string {
    switch (temperature) {
      case 'hot':
        return 'داغ';
      case 'warm':
   ...
   */
  sortLeadsByTemperaturePriority(leads: Lead[]): Lead[] {
    const temperatureOrder = { 'hot': 0, 'warm': 1, 'cold': 2 };
    
    return leads.sort((a, b) => {
      const aTemp = leadTemperatureService.calculateLeadTemperature(a);
      const bTemp = leadTemperatureService.calculateLeadTemperature(b);
